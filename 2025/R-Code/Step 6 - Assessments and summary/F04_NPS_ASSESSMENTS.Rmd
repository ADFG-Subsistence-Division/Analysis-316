...
resHealthData_raw <- read_csv('../../CSV/01 - Database Extract/REC376_raw.csv')


cat(formatSummaryBlock(paste("Opening file: REC376_raw.csv, ", 
                             nrow(resHealthData_raw), 
                             " records loaded.", sep="")))
```
...
resHealthData <- resHealthData_raw %>% 
  left_join(sampleData, by = c("projID", "studyear", "communty")) %>% 
  left_join(resNameData, by = c("resource"))
...
# Aggregate to HH level.
resHealthData <-group_by(resHealthData, projID, studyear,communty, commname, commhh, samphh, HHID, resource, resName) %>%
...
#Aggregate to community level.
resHealthData <-group_by(resHealthData, projID, studyear,communty, commname, commhh, samphh, resource,resName) %>%
  summarize (no = sum(no,na.rm = TRUE),
             yes = sum(yes,na.rm = TRUE),
             unknown = sum(unknown,na.rm = TRUE),
             missing = sum(missing, na.rm = TRUE))
...
resHealthDescData <- resHealthData_raw

# Aggregate to HH level.
resHealthDescData <- resHealthData_raw %>% 
  mutate(commname = "Ambler",
         commhh = unique(NPSData$commhh),
         samphh = unique(NPSData$samphh)) %>% 
  group_by(projID, studyear,communty, commname, commhh, samphh, HHID, resource, resSafeDesc) %>%
  summarize(filterq = sum(filterq, na.rm = TRUE)) %>% 
  filter(!is.na(resource)) %>% 
  left_join(resNameData, by = c("resource")) %>% 
  select(communty:resource,resName,resSafeDesc)
...
resYNTblData <- filter (tmpResYNData, commname == comm) %>% ungroup() %>%
               select(resName, samphh, no, yes, unknown, missing) 
...
firewoodData <- filter(NPSData, displayGroup == 66172)
...
Note: for Ambler (2024), there were very few responses to environmental impact questions. The responses are reported values rather than coded and summarized values.
...
```{r environmental impact - sparse data}
# Filter to the environmental impact question.
environData2 <- NPSData %>% select(projID:HHID, environChangesYN:environChangesCD2) %>% 
  filter(environChangesYN == 1) %>% 
  distinct(HHID, environChangesDesc) %>% 
  bind_cols(sampleData %>% select(projID:communty, commname, samphh, commhh),.)
```
...
```{r environmental impact table review, eval = FALSE}
...
```{r environmental impact table - sparse data}
tmpEnvironDescData <- environData2
...
# ##############################################################################
  ...
  rio::export(naturalMatData, fName)
      ...
      fName = str_interp('../../CSV/05 - Final Analysis Output/naturalMaterialLocations_raw.csv')
  cat(formatSummaryBlock(
    paste('Writing file: ', fName, 
          ' ', nrow(natMatLocations), ' records to be written', sep='')))
  
  # For 2023; we are, by default, working exclusively with R code..
  rio::export(natMatLocations, fName)
  ...
  cat(formatSummaryBlock(
    paste('Writing file: ', fName, 
          ' ', nrow(environDescTblData), ' records to be written', sep='')))
  
  # For 2023; we are, by default, working exclusively with R code..
  rio::export(environDescTblData, fName)
