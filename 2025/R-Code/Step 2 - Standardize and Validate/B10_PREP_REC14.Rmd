...
unspecTable <- REC14StdData %>% filter(str_detect(resName, "Unspecified") & harvestq == 1) %>% 
  ungroup() %>% 
...
# One respondent gave an unclear response about their harvest of red squirrel; they reported normally harvesting red squirrel but could not recall if they harvested them during the study year. Removing this record.

REC14CSISData <- REC14CSISData %>% filter(!is.na(used))
...
# Write out unspecified resource conversion factor table for binding to convFact.csv
  fName = str_interp('../../CSV/03 - main/REC14_calc_convFact.csv')
  cat(formatSummaryBlock(
    paste('Writing file: ', fName, 
