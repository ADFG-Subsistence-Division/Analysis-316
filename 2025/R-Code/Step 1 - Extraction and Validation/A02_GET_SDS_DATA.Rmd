...
### Extract defined display group names

```{r extract defined display group names}
...
strSQL = sql(str_interp('SELECT [projID]
                        ...
                        ,[page]
                        ,[order]
                        FROM [Sub_SDS].[dbo].[vw_displayGroupsByCommunity]
                        WHERE projID = 0 OR (projID = ${projID} AND studyear = ${studyear})'))

lookupCodeSets[['definedDisplayGroups']] <- odbc::dbGetQuery(conn, strSQL)

count <- nrow(lookupCodeSets[['definedDisplayGroups']])
cat(formatSummaryBlock(paste("SQL Extract:", strSQL ,", ", 
                             count, 
                             " records loaded.", sep="")))
