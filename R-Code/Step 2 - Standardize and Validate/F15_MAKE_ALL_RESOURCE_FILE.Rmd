...
# Read raw conversion factor file data.
convFactData_raw <- read.csv(str_interp('../../CSV/03 - Main/convFact.csv'), 
                         na = '', header = TRUE, strip.white = TRUE) 

# Load data for calculations & attempt to merge.
convFactData_final <- tryCatch(
  {
    ...
    calcConvFact <-
      list.files(path = "../../CSV/03 - Main/", 
                 ...
                 full.names = TRUE) %>% 
      lapply(function(f) {
        read_csv(f,col_types = c("cicciddddc"))
      }) %>% 
      bind_rows()
...
convFactData <- convFactData_final %>% 
        select(projID, studyear, communty, resource, units, convFact, defaultUnits) 
...
# Dolly Varden 125006000 was causing duplication of Dolly Varden, unknown type harvests. Recoding specList to avoid duplication.

harvData <- harvData %>% mutate(specList = if_else(resource == 125006000, 2, specList))

# Market resources are showing up for some reason, and causing NAs.

harvData <- harvData %>% filter(!is.na(commname))
