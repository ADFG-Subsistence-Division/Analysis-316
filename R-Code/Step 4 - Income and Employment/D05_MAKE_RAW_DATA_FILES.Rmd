...
ggplot(plotMedIncData, aes(x = factor(commname, 
                                      levels = c("United States", "Alaska", setdiff(unique(commname), c("United States", "Alaska")))), 
                           y = estIncome_median)) +
  geom_col(fill = "steelblue") +
  geom_errorbar(aes(ymin = lower_medIncome, ymax = upper_medIncome), width = 0.2) +
  ggtitle(str_c("Median Income ", str_c(medianEmployData$commname, collapse = ", "), ", ", medianEmployData$studyear)) +
  labs(
    x = "",
    y = "Median Income (USD)"
  ) +
