...
# Programmer: D. Koster
# Date: 02/2022
# file: f_standardizeHHRecords.r
# Purpose: Standardized HH records ensures that all detail added for any 
#          household beyond what is supplied on the printed form, ie: a new
#          resource, is added to all other households. This script was built
#          with harvest resources in mind, but can be used for any instance
#          where all households should have the same details for simplification 
#          of statistical analysis, such as creation of means.
#
# Changelog: [Programmer | Date | Change Decription | Template Update [Y|N] | [One-off]]
# Programmer:
#
#
# Implementation notes: Note, prior to standardizing resource codes, we must
...
#
# For the new standardize function; we have to utilize the detail function above.
#

# Given: projectKey; hhKey, replicationList, starting data:
# We need to: Create a 'template' record for each unique replication
#             key value 
# Such That: All replication values are present for each household 
#            for a projID/studyear/communty
# AND: replication key values not appearing in a projID/studyear/communty
#      are not added
# How do we accomplish this?
#
# Create a set of all rows appearing in the dataset and containing the
#    replication key, projectKey, and hhKey.
#
# - Loop Through each Household 
# - Limit the list of replication keys to just those with matching 
#     projID, studyear, communty
# - Add the HHID and strata value to the template list
# - bind_rows into a placeholder data-frame.
#
