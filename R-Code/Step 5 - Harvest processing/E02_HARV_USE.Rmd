...
walk(commnameList, function(comm) {
  ...
  # Pie chart: Remove total and unused
  pieData <- tempData %>%
    filter(resName != 'All resources', percap > 0) %>%
    arrange(percap) %>%
    mutate(resName = factor(resName, levels = rev(unique(resName)))) %>% 
  # Calculate percentage and label positions for outside labeling
    mutate(
      pct = round(percap / sum(percap) * 100,0),
      ymax = cumsum(pct),
      ymin = lag(ymax, default = 0),
      mid = (ymax + ymin) / 2,
      # Place the labels outside at 1.2x the radius of the chart
      label_x = 1.2 * sin(2 * pi * mid),
      # Place the labels halfway between ymax and ymin in radians
      label_y = 1.2 * cos(2 * pi * mid)
    )
  
  colrs <- getColors(nColors = nrow(pieData))

  catPlot <- ggplot(pieData, 
                    aes(x = "", y = pct, fill = resName)) +
    geom_col(width = 1, color = "white") +
    coord_polar(theta = "y", start = 0, direction = -1) +
    ggtitle(str_interp("Harvest by category ${comm}, ${unique(harvUseCatData$studyear[harvUseCatData$commname == comm])}.")) +
    xlab("") + ylab("") +
    scale_fill_manual(values = colrs, labels = paste0(pieData$resName,"; ",pieData$pct,"%")) +
    theme_void() +
    # geom_text(
    #   aes(x = label_x, y = label_y, label = paste0(pct,"%")),
    # inherit.aes = FALSE,
    # size = 5
    # ) +
    theme(legend.title = element_blank(), text = element_text(family = "serif")) +
    guides(fill = guide_legend(title = "Resource category"))
...
# harvUseCatData <- filter(harvRawData, specList == 2) %>%
#   select(projID, studyear, communty, commname, resource, resName, 
#          used_pct, attempt_pct, harvestq_pct, estHarvestAmt_sum, estHarvestLbs_sum, percap)
# 
...
# {
# 
#   tempData <- filter(harvUseCatData, commname == comm) %>%
#     arrange(resource) %>%
#     select(resName, estHarvestLbs_sum, percap)
#   
#   tempData$estHarvestLbs_sum <- round(tempData$estHarvestLbs_sum, 1)
#   tempData$percap <- round(tempData$percap, 1)
#   
#   tblOut <- kbl(tempData,
#       caption=formatTableHeader(str_interp("Harvest by category for ${comm}, ${studyear}")),
...
#   print(tblOut)
#   
#   # For the figure, we need to remove the total row.
#   tempData <- filter(tempData, resName != 'All resources')
#   
#   # Omit non-harvested species for clarity.
#   tempData <- filter(tempData, percap > 0.000)
#   # Organize data for more logical display.
...
#   
#   catPlot <- ggplot(tempData, 
#          aes(x = "", 
#              y = percap, 
#              fill = resName)) +
#     ggtitle(str_interp("Harvest by category ${comm}, ${studyear}.")) +
#     geom_bar(width = 1, 
#              stat = "identity", 
#              color = "white") +
#     coord_polar("y", 
...
#     select(resName, used_pct, attempt_pct, harvestq_pct)
#   
#   #usePlot <- ggplot(data, aes(fill=condition, y=value, x=resName)) + 
#   #  geom_bar(position="dodge", stat="identity")
# 
#   tempData <- filter(harvUseCatData, commname == comm & resource != 0)
#   count = nrow(tempData)
#   
#   useVar <- c(tempData$used_pct, tempData$attempt_pct, tempData$harvestq_pct)
#   label <- c(rep("using", count), rep("attempting", count), rep("harvesting", count))
#   # Repeat each category 3 times?
#   category <- rep(tempData$resName, 3)  
#   plotData <- data.frame(category, label, useVar)
#   
#   labelOrder = c("using", "attempting", "harvesting")
#   plotData$category = str_wrap(plotData$category, width = 10)
#   catOrder <- distinct(plotData, category)$category
#       
#   colrs <- getColors(nColors = 3)
#   plotOut <- ggplot(plotData, aes(fill=factor(label, labelOrder), 
#                                   y=useVar, 
#                                   x=category,
#                        label = scales::percent(useVar))) + 
#     ggtitle(str_interp("Use, attempt, and harvest by resource category, ${comm}, ${studyear}")) +
#     geom_bar(position = position_dodge(width = 0.5), stat="identity") +
...
```{r salmon species pie, eval = FALSE}

for(comm in commnameList$commname)
{
  tempData <- filter(harvUseFigData, catCode == 11 & 
                       resource != 110000000 &
                       commname == comm) 
  ...
  if(nrow(tempData) > 0)
  {
     ...
     specPlot <- ggplot(tempData, 
           aes(x = "", 
               y = percap, 
               fill = resName)) +
      ggtitle(str_interp("Salmon harvest by species ${comm}, ${studyear}.")) +
      geom_bar(width = 1, 
               stat = "identity", 
               color = "white") +
      coord_polar("y", 
    ...
    cat(warningMessage(str_interp("No salmon data present for ${comm}, ${studyear}")))
  }
}
...
```{r non salmon species pie, eval = FALSE}

for(comm in commnameList$commname)
{
  tempData <- filter(harvUseFigData, catCode == 12 & 
                       resource != 120000000 &
                       commname == comm) 
  ...
  if(nrow(tempData) > 0)
  {
     ...
     specPlot <- ggplot(tempData, 
           aes(x = "", 
               y = percap, 
               fill = resName)) +
      ggtitle(str_interp("Non-salmon harvest by species ${comm}, ${studyear}.")) +
      geom_bar(width = 1, 
               stat = "identity", 
               color = "white") +
      coord_polar("y", 
    ...
    cat(warningMessage(str_interp("No non-salmon data present for ${comm}, ${studyear}")))
  }
}
...
```{r large game species pie, eval = FALSE}

for(comm in commnameList$commname)
{
  tempData <- filter(harvUseFigData, catCode == 21 & 
                       resource != 210000000 &
                       commname == comm) 
  ...
  if(nrow(tempData) > 0)
  {
     ...
     specPlot <- ggplot(tempData, 
           aes(x = "", 
               y = percap, 
               fill = resName)) +
      ggtitle(str_interp("Large land mammal harvest by species ${comm}, ${studyear}.")) +
      geom_bar(width = 1, 
               stat = "identity", 
               color = "white") +
      coord_polar("y", 
    ...
    cat(warningMessage(str_interp("No large land mammal data present for ${comm}, ${studyear}")))
  }
}
...
```{r small game species pie, eval = FALSE}

for(comm in commnameList$commname)
{
  tempData <- filter(harvUseFigData, catCode == 22 & 
                       resource != 220000000 &
                       commname == comm) 
  ...
  if(nrow(tempData) > 0)
  {
     ...
     specPlot <- ggplot(tempData, 
           aes(x = "", 
               y = estHarvestAmt_sum, 
               fill = resName)) +
      ggtitle(str_interp("Small land mammal harvest by species ${comm}, ${studyear}.")) +
      geom_bar(width = 1, 
               stat = "identity", 
               color = "white") +
      coord_polar("y", 
    ...
    cat(warningMessage(str_interp("No small land mammal data present for ${comm}, ${studyear}")))
  }
}
...
```{r feral species pie, eval = FALSE}

for(comm in commnameList$commname)
{
  tempData <- filter(harvUseFigData, catCode == 23 & 
                       resource != 230000000 &
                       commname == comm) 
  ...
  if(nrow(tempData) > 0)
  {
     ...
     specPlot <- ggplot(tempData, 
           aes(x = "", 
               y = percap, 
               fill = resName)) +
      ggtitle(str_interp("feral land mammal harvest by species ${comm}, ${studyear}.")) +
      geom_bar(width = 1, 
               stat = "identity", 
               color = "white") +
      coord_polar("y", 
    ...
    cat(warningMessage(str_interp("No feral land mammal data present for ${comm}, ${studyear}")))
  }
}
...
```{r marine mammal species pie, eval = FALSE}

for(comm in commnameList$commname)
{
  tempData <- filter(harvUseFigData, catCode == 30 & 
                       resource != 300000000 &
                       commname == comm) 
  ...
  if(nrow(tempData) > 0)
  {
     ...
     specPlot <- ggplot(tempData, 
           aes(x = "", 
               y = percap, 
               fill = resName)) +
      ggtitle(str_interp("Marine mammal harvest by species ${comm}, ${studyear}.")) +
      geom_bar(width = 1, 
               stat = "identity", 
               color = "white") +
      coord_polar("y", 
    ...
    cat(warningMessage(str_interp("No marine mammal data present for ${comm}, ${studyear}")))
  }
}
...
```{r migratory bird species pie, eval = FALSE}
...
```{r marine invertebrate species pie, eval = FALSE}

for(comm in commnameList$commname)
{
  tempData <- filter(harvUseFigData, catCode == 50 & 
                       resource != 500000000 &
                       commname == comm) 
  ...
  if(nrow(tempData) > 0)
  {
     ...
     specPlot <- ggplot(tempData, 
           aes(x = "", 
               y = percap, 
               fill = resName)) +
      ggtitle(str_interp("Marine invertebrate harvest by species ${comm}, ${studyear}.")) +
      geom_bar(width = 1, 
               stat = "identity", 
               color = "white") +
      coord_polar("y", 
    ...
    cat(warningMessage(str_interp("No marine invertebrate data present for ${comm}, ${studyear}")))
  }
}
...
```{r vegetation species pie, eval = FALSE}

for(comm in commnameList$commname)
{
  tempData <- filter(harvUseFigData, catCode == 60 & 
                       resource != 600000000 &
                       commname == comm) 
  ...
  if(nrow(tempData) > 0)
  {
     ...
     specPlot <- ggplot(tempData, 
           aes(x = "", 
               y = percap, 
               fill = resName)) +
      ggtitle(str_interp("Vegetation harvest by species ${comm}, ${studyear}.")) +
      geom_bar(width = 1, 
               stat = "identity", 
               color = "white") +
      coord_polar("y", 
    ...
    cat(warningMessage(str_interp("No vegetation data present for ${comm}, ${studyear}")))
  }
}
