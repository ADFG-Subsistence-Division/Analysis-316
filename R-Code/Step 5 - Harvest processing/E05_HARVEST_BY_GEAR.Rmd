...
for(comm in commnameList$commname)
{
  ...
  print(tmpFigData)
  colrs <- getColors(nColors = nrow(distinct(tmpFigData, gearName))
...
```{r non salmon figures, eval = FALSE}
...
for(comm in commnameList$commname)
{
  ...
  # Start with initial plot information.
  plotOut <- ggplot(tmpFigData, aes(x=reorder(resName, desc(harvRank)), y=harvestLbs)) +
  ggtitle(str_interp("Harvests of nonsalmon by gear type ${comm}, ${studyear}")) +
      geom_col(aes(fill = gearName), width=0.7) +
      scale_fill_manual(values = colrs)
...
## Nonsalmon harvest by gear type
```{r nonsalmon harvest by gear type}
