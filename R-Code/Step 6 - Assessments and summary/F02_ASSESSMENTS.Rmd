---
title: "F02_ASSESSMENTS - (316) NPS Ambler Comprehensive"
...
lsmDisplayGroups <- c('6604', '6606', '6608', '6610', '6612', 
                      '6614', '6615', '6617', '6618')
...
lsmCodeList <- codeSetData %>% 
  ...
  mutate(codeDescription = factor(codeDescription, 
                                  levels = c('Less', 'Same', 'More', 'Did not use'),
                                  ordered = TRUE)) %>%
  rename('histUse' = 'code',
...
lsmReasonLessList <- tibble(
  ...
  mutate(lessReason = factor(lessReason, ordered = TRUE))
...
# Merge in histUse codes for pivoting later on.
...
commnameList <- ungroup(lsmOutputData) %>% select(commname) %>% distinct(commname)
LSMtempData <- lsmOutputData

# Calculate percentages and ensure NA gets turned to 0.
LSMtempData$anyUse_pct = LSMtempData$anyUse/LSMtempData$validLSM
LSMtempData$less_pct = LSMtempData$less/LSMtempData$validLSM
LSMtempData$same_pct = LSMtempData$same/LSMtempData$validLSM
LSMtempData$more_pct = LSMtempData$more/LSMtempData$validLSM
LSMtempData$DontUse_pct = LSMtempData$DontUse/LSMtempData$validLSM
