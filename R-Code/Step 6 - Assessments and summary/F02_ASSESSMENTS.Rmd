...
moreReasonsHeat <- moreReasonsLong %>%
 group_by(resName) %>% 
 mutate(
    row_max = max(numResponses, na.rm = TRUE),
    fill_alpha = ifelse(row_max == 0, 0, moreUse / row_max), # avoid div by zero
    fill_col = scales::alpha("red", fill_alpha)              # alpha sets transparency
  ) %>%
  ungroup()
...
pwalk(
 ...
 plotOut <- ggplot(df_filt, aes(x = moreReason, y = resName)) +
   geom_tile(aes(fill = fill_col, alpha = fill_alpha), color = "white", na.rm = TRUE) +
   geom_text(aes(label = numResponses), color = "black", fontface = "bold") +
   scale_fill_identity() +
   ...
   labs(x = "Reasons for more use", y = "Resource Name", title = paste0("Reasons for more use by resource category, ",commname_val,", ",studyear_val))
