...
# 
# # Aggregate to HH level.
# resHealthDescData <- group_by(resHealthDescData, projID, studyear,communty, commname, commhh, samphh, HHID, resource, resName, resWhy) %>%
...
```{r environmental impact, eval = FALSE}
...
# Filter to the environmental impact question.
environData <- NPSData %>% select(projID:HHID, environChangesYN:environChangesCD2) %>% 
  filter(environChangesYN == 1) %>% 
...
tmpEnvironDescData <- environData
