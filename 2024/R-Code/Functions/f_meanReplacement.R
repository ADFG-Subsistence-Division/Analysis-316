...
# Programmer: D. Koster
# Date: 02/2022
# file: f_standardizeConversionFactors.r
# Purpose: Standardized conversion factor routines; each function addresses a
#          different set of circumstances, because these are functions to be
#          accessed, rather than copied annually, we can do a better job of
#          controlling code & code updates. This also prevents errors from being
#          reintroduced from old code.
#
#
#
...
#          "projID", "studyear", "communty", "resource", "units"
#
# convData:
#    must contain the following columns:
#          "projID", "studyear", "communty", "resource", "units", "convFact","defaultUnits"
#
# -- FUTURE TO-DO: Make a generic replacement function that uses 'type' to 
#                  choose minimum|mean|mode|median
#
# ##############################################################################

# Need for mean replacement?

# key information + strata
# column name
...
meanReplaceStratified <- function(sourceData, replCol, checkCol = "harvestq")
{
  ...
  # 1.9 If all else fails, use the minimum of '1' and return the mean replaced
...
# Generic mean replacement for use with non-SDS data where stratification or
#   circumstances where the goal is not harvest by resource. is not
#   present - this allows for a more generic specification as well. This is not
#   meant for use when multiple-levels of replacement are necessary, such as 
#   sex with large game.
