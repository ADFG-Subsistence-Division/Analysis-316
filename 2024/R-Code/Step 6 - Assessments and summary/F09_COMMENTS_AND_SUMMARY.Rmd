---
title: "F09_COMMENTS_AND_SUMMARY - (316) NPS Ambler Comprehensive"
...
<div class="title-container">
  ...
  <div class="title">F09_COMMENTS_AND_SUMMARY - (316) NPS Ambler Comprehensive</div>
</div>
...
# Raw comments data
commentData <- read.csv('../../CSV/01 - Database Extract/REC300_raw.csv',
                   na = '', 
                   header = TRUE, 
                   strip.white = TRUE)

count <- nrow(commentData)
cat(formatSummaryBlock(paste("Opening file: REC300_raw.csv, ", 
                             count, 
                             " records loaded.", sep="")))
...
commentData <- left_join(commentData, communtyLkpData, by=c("communty")) %>%
  select(projID, studyear, communty, commname, comments, summary)
...
knitr::kable(commentData,
             caption=formatTableHeader("Comments and summary"))
...
# Sample code for writing out a file.
  fName = str_interp('../../CSV/05 - Final Analysis Output/Comments and Summary.csv')
  cat(formatSummaryBlock(
    paste('Writing file: ', fName, 
          ' ', nrow(commentData), ' records to be written', sep='')))

# Standard is to use CSV files for portability.
  rio::export(commentData, fName)

```

<p class="h1footer"> End of F09 COMMENTS and SUMMARY script. </p>
